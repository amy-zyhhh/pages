...
- 首页按分类展示导航卡片，支持置顶入口和本页搜索。
- 搜索支持点击“搜索”按钮或按回车提交；按 `/` 可以展开并聚焦当前页面搜索框。
- 首页搜索框手动输入 `/关键词` 后提交，可以跳转到网页搜索。
- 加密页展示私有导航，并可同时搜索加密导航、blogs 和 InfoRSS。
...
- 首页：只搜索首页当前导航链接；手动输入 `/关键词` 并提交时可触发网页搜索。
- 加密页：搜索加密导航、blogs 和 InfoRSS；链接结果优先显示，blogs 与 InfoRSS 使用文章预览样式。
...
这些事项不是当前必须完成的问题，而是后续继续扩展时最值得优先考虑的改进。
...
2. 建立独立搜索索引
   ...
   当前搜索文本主要随卡片写入页面 DOM。内容继续增多后，可以在构建阶段生成轻量 JSON 搜索索引，页面按需加载。这样加密页搜索 InfoRSS、Blogs 全文检索和 InfoRSS 正文检索都会更轻。
...
5. 优化移动端筛选面板
   ...
   InfoRSS 的搜索范围、日期、分类和清空按钮在移动端仍然偏功能密集。可以把搜索、范围、日期和分类分组展示，保持顶部只放图标，展开后再显示完整筛选面板。

6. 增加抓取状态页或日志摘要

   可以增加一个本地状态页面或日志摘要，显示最近一次抓取时间、新增数量、跳过重复数量、错误数量和错误来源。这样排查计划任务、网络异常和接口变化会更直观。
...
8. 增加跨设备阅读状态同步
   ...
   如果以后需要同步“已读/未读”状态，可以在本地 `localStorage` 之外增加可选同步层，例如轻量数据库或远程 JSON 存储。同步内容建议只保存文章链接的标识和阅读状态，不保存敏感正文。
